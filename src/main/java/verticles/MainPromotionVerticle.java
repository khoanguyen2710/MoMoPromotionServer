...
import db.PromotionErrorControlDb;
...
/**
 * Created by khoanguyen on 12/31/16.
 */
public class MainPromotionVerticle extends AbstractVerticle {
    ...
    protected ErrorPayBackDb errorPayBackDb;
    @Override
    public void start() {
        glbConfig = config();
        ...
        errorPayBackDb = new ErrorPayBackDb(vertx, logger);
    }

    public void checkDevice(boolean checkDevice, String deviceInfo, String os, int appCode, String program, int phoneNumber, String imei, boolean checkGmail, boolean checkSim, String className, Common.BuildLog log, final Handler<JsonObject> callBack) {
        ...
        if (checkDevice) {
            ...
            callBack.handle(new JsonObject().put(StringConstUtil.PromotionField.ERROR, 0));
        }
    ...
    protected Map<String, Long> getListManyVouchers(String voucherInfo) {
        String[] listGift = voucherInfo.split(";");
        Map<String, Long> listVoucher = new HashMap<>();
        String[] gift = {};
        for (String giftInfo : listGift) {
            gift = giftInfo.trim().split(":");
            if (gift.length == 2 && DataUtil.strToLong(gift[1].trim()) > 0) {
                listVoucher.put(gift[0].trim(), DataUtil.strToLong(gift[1].trim()));
            }
        }
        return listVoucher;
    }

    protected long calculateTotalGiftAmount(Map<String, Long> listVouchers) {
        long totalAmount = 0;

        Collection<Long> collectionAmounts = listVouchers.values();
        for (Long amount : collectionAmounts) {
...
//                    }
    ...
    protected void sendNotiViaBroadcastWithType(String title, String content, String phoneNumber, int type, int screenId) {
        Notification noti = new Notification();
        noti.priority = 2;
        noti.type = 28;
        noti.caption = title;// "Nhận thưởng quà khuyến mãi";
        ...
        noti.extra = new JsonObject()
                .putNumber("type", type)
                .putNumber("screenId", screenId).toString();
        noti.receiverNumber = DataUtil.strToInt(phoneNumber);
        Misc.sendNotiFromTool(vertx, noti);
    }
    ...
    protected void giveVoucher(
            final long totalGiftAmount
            , final int giftTime
            , final String agent
    ...
    protected void giveListVouchers(
            final long totalGiftAmount
            , final int giftTime
            , final String agent
            , final String phoneNumber
            , final String program
            , final Map<String, Long> listGiftName
            , final Common.BuildLog log
            , final String className
            , final boolean activeGift
            , final Handler<JsonObject> callback) {
                            ...
                            vertx.setPeriodic(250L, new Handler<Long>() {
                                ...
                                public void handle(Long aPeriodicLong) {
                                    if (atomicInteger.decrementAndGet() < 0) {
                                        ...
                                        final int itemPosition = atomicInteger.intValue();
                                        addLog("info", "itemPosition " + itemPosition, className, log);
                                        Set<String> setGifts = listGiftName.keySet();
                                        giftType.setModelId(setGifts.toArray()[itemPosition].toString());
                                        giftManager.createLocalGiftForBillPayPromoWithDetailGift(phoneNumber
                                                , listGiftName.get(setGifts.toArray()[itemPosition].toString())
                                                , giftType
                                                , promotedTranId
                                                , agent
                                                , modifyDate
                                                ...
                                                , keyValues, note, new Handler<JsonObject>() {
                                                    ...
                                                    public void handle(JsonObject jsonObject) {
                                                        ...
                                                        if (err == 0) {
                                                            addLog("info", "Tao qua local thanh cong", className, log);
                                                            /*******/
//                                                            V2Utils.sendGift(vertx, logger, gift);
                                                            if (activeGift) {
                                                                giftManager.useGift(phoneNumber, giftId, new Handler<JsonObject>() {
                                                                    @Override
                                                                    public void handle(JsonObject jsonObject) {
                                                            ...
                                                            addLog("info", "Tao qua local that bai", className, log);
                                                            joReply.putNumber(StringConstUtil.PromotionField.ERROR, PromoContentNotification.Error.ERROR_CREATE_LOCAL_GIFT.index);
                                                            joReply.putNumber(StringConstUtil.PromotionField.GIFT_TID, promotedTranId);
                                                            joReply.putString(StringConstUtil.PromotionField.DESCRIPTION, PromoContentNotification.Error.ERROR_CREATE_LOCAL_GIFT.info);
                                                            joReply.putNumber(StringConstUtil.PromotionField.GIFT_POSITION, itemPosition);
                                                            callback.handle(joReply);
                                                            return;
                                                        }
                            ...
                            addLog("info", "Core loi", className, log);
                            addLog("info", SoapError.getDesc(error), className, log);
                            callback.handle(new JsonObject().putNumber("error_core", error).putNumber(StringConstUtil.PromotionField.ERROR, PromoContentNotification.Error.CORE_ERROR.index)
                                    .putString(StringConstUtil.PromotionField.DESCRIPTION, PromoContentNotification.Error.CORE_ERROR.info)
                                    .putNumber(StringConstUtil.PromotionField.GIFT_TID, promotedTranId));
                            return;
                        }
                    }
    ...
    protected void executePayBackAuto(final JsonObject joReplyPromotion, int error, final PhonesDb.Obj phoneObj,final String address) {
        errorPayBackDb.findOne(StringConstUtil.WomanNationalField.PROGRAM + "_" + error, new Handler<ErrorPayBackDb.Obj>() {
            @Override
            public void handle(ErrorPayBackDb.Obj errorPayBackObj) {
                if (errorPayBackObj != null) {
                    //This error is sent to redis.
                    joReplyPromotion.putString(StringConstUtil.PromotionField.GROUP, StringConstUtil.WomanNationalField.PROGRAM)
                            .putString(StringConstUtil.PromotionField.PHONE_NUMBER, "0" + phoneObj.number)
                            .putString(StringConstUtil.PromotionField.ADDRESS, address)
                            .putString(StringConstUtil.BroadcastField.COMMAND, NotificationRedisVerticle.INSERT_ERROR_PROMOTION_TRANS);
                    vertx.eventBus().sendWithTimeout(AppConstant.RedisVerticle_NOTIFICATION_FROM_TOOL, joReplyPromotion, 30000L, new Handler<AsyncResult<Message<JsonObject>>>() {
                        @Override
                        public void handle(AsyncResult<Message<JsonObject>> msg) {
                            if(msg.failed())
                            {
                                logger.info("RedisVerticle_NOTIFICATION_FROM_TOOL TIME OUT");
                            }
                            else {
                                JsonObject joReceived = msg.result().body();
                                logger.info("RedisVerticle_NOTIFICATION_FROM_TOOL LKTK SUCCESS " + joReceived.toString());
                            }
                        }
                    });
                }
